...
export function createSubscriptionApply(deps: SubscriptionApplyDeps) {
  ...
  const ensureSubscriptionForNode = async (node: CloudNode) => {
    ...
    // IP (typical on China Mobile cellular). Only added when:
    ...
    // probe interval pays the cost of a known-bad path (DNS-poisoned, TLS
    ...
    // what's broken on the networks M1 is meant to fix (CN cellular
    // DNS-poisons both *.workers.dev and the custom hostname). A
    // 'failed' verdict under those conditions is a false negative, and
    // falling back to workers.dev pushes traffic onto a more poisoned
