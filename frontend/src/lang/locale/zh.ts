export default {
  ...
  cloud: {
    provider: {
      ...
      categoryByo: '自带服务器',
      categoryByoHint:
        'PrivateDeploy 通过 SSH 接入你已有的 VPS 并在上面部署协议栈，VPS 本身需自行准备。',
    },
    credentials: {
      title: '云凭据',
    ...
    nodes: {
      ...
      repairConfirm:
        '确定修复「{label}」？软件会检查并重新关联云防火墙，必要时重启原服务器，再验证服务端口可达性；不会创建新的计费主机。',
    ...
    progress: {
      requesting: '部署请求处理中',
      providerPending: '等待云厂商返回最终状态',
      applyingLocal: '云主机已返回，正在写入本地配置',
      delayed: '云厂商处理时间超过预期',
      uncertain: '创建结果尚未确认',
      cancelHint: '“停止等待”不会删除云端已开始创建的服务器；服务器可能继续创建并计费，软件随后会刷新列表。',
      cancel: '停止等待',
      cancelling: '正在停止等待…',
      cancelled: '已停止等待并刷新列表。云服务器仍可能继续创建和计费，请到云厂商控制台确认。',
      delayedHint: '请刷新节点列表并在云厂商控制台核对；不要重复点击部署。',
      uncertainHint: '请求可能已在云端执行。请先刷新并核对云厂商控制台，再决定是否重试。',
    },
    table: {
      label: '名称',
      region: '区域',
    ...
    manual: {
      ...
      addDescription:
        '至少填写一组协议参数，例如 Shadowsocks 端口和密码，或 Hysteria/VLESS/Trojan 配置。',
  ...
  plugins: {
    ...
    fullTrustTitle: '插件以完全信任方式运行',
    fullTrustWarning:
      '该插件将以与应用本身相同的权限运行——没有任何沙箱隔离。\n\n添加并启用后，它可以：\n· 读取和修改你的文件\n· 访问网络并向任意地址发送数据\n· 在本机执行命令\n\n请仅在信任插件作者及其来源时继续。',
    fullTrustAccept: '我已了解风险，仍要添加',
    testRun: '运行测试',
    deprecated: '已废弃',
    newVersion: '新版本',
  },
  ...
  cdn: {
    ...
    setup: {
      ...
      step3Body:
        '验证通过后，在任意云节点上点击 "部署 CDN" 即可上传 Worker 作为该节点的 CDN 前端。',
    },
    tokenInput: {
    ...
    account: {
      ...
      example: '示例 URL',
      noSubdomain:
        '尚未申领 workers.dev 子域名。请先在 Cloudflare Workers 控制台申领一次，然后重新验证。',
    },
    node: {
      deploy: '部署 CDN',
      deploying: '部署中…',
    ...
    confirm: {
      clear:
        '清除已保存的 Token 和所有 Worker 部署记录？已上传到 Cloudflare 的 Worker 不会被自动删除。',
    ...
    customDomain: {
      ...
      zonePlaceholder: '加载中…',
      noZones:
        '当前 Token 没有可见的活跃域名。请先在 Cloudflare 添加站点，并确认 Token 拥有对应的 Zone 权限。',
