export default {
  ...
  cloud: {
    ...
    nodes: {
      ...
      repairConfirm:
        'Repair "{label}"? PrivateDeploy will reconcile its cloud firewall, reboot the same server only if needed, and verify service-port reachability. No additional billed server will be created.',
    ...
    progress: {
      requesting: 'Deployment request is still processing',
      providerPending: 'Waiting for the provider to return a final state',
      applyingLocal: 'The server returned; applying its local configuration',
      delayed: 'The provider is taking longer than expected',
      uncertain: 'The create result is not confirmed',
      cancelHint: 'Stopping the wait does not delete a server the provider already started creating. It may still finish and incur charges; the list will refresh next.',
      cancel: 'Stop waiting',
      cancelling: 'Stopping wait…',
      cancelled: 'Stopped waiting and refreshed the list. The server may still be created and billed; verify it in the provider console.',
      delayedHint: 'Refresh the node list and check the provider console. Do not click Deploy again.',
      uncertainHint: 'The request may have completed remotely. Refresh and check the provider console before retrying.',
    },
    table: {
      label: 'Label',
      region: 'Region',
