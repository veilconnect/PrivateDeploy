...
func TestListInstancesFallsBackToLocalNodeRecordsWithoutAPIKey(t *testing.T) {
	...
	if instances[0].IPv4 != "198.51.100.20" {
...
func TestListInstancesMigratesReplacedRecordByIPAndRecoversUserData(t *testing.T) {
	...
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		case "/instances":
			...
			_, _ = w.Write([]byte(`{"instances":[{"id":"inst-new","label":"sgp-node","status":"active","region":"sgp","main_ip":"198.51.100.15","created_at":"2026-04-03T10:00:00Z"}]}`))
