...
type RegionScore struct {
	...
	GFWRisk   string   `json:"gfwRisk"`   // low, medium, high, critical
	AIAccess  bool     `json:"aiAccess"`  // can reach OpenAI, Claude, etc.
	Score     float64  `json:"score"`     // 0-100
	Reasons   []string `json:"reasons"`
}

// gfwRiskMap classifies regions by GFW risk level.
var gfwRiskMap = map[string]string{
...
func ScoreRegions(regions []Region, latencies map[string]float64) []RegionScore {
	...
	for _, r := range regions {
		...
		// GFW risk: 30% weight
		riskScore := scoreGFWRisk(rs.GFWRisk)
		score += riskScore * 0.30
		if riskScore >= 80 {
			reasons = append(reasons, "GFW 风险低")
		} else if riskScore <= 30 {
			reasons = append(reasons, "GFW 风险高")
		}
...
func getGFWRisk(regionID string) string {
	id := strings.ToLower(regionID)
	if risk, ok := gfwRiskMap[id]; ok {
		return risk
	}
	return "medium"
}
...
func scoreGFWRisk(risk string) float64 {
	switch risk {
	case "low":
		return 100
	case "medium":
