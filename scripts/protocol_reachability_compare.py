...
def launch_singbox(singbox_path: Path, outbound: Dict[str, Any], socks_port: int, tmpdir: Path) -> subprocess.Popen:
    ...
    cfg_path = tmpdir / "singbox-reachability.json"
...
def run_reachability_for_target(
    singbox_path: Path,
    node: Dict[str, Any],
    ip: str,
    ...
    with tempfile.TemporaryDirectory(prefix="protocol-reachability-") as temp_dir:
...
def write_outputs(rows: List[Dict[str, Any]], output_dir: Path) -> Dict[str, Path]:
    ...
    json_path = output_dir / f"protocol_reachability_compare_{timestamp}.json"
    tsv_path = output_dir / f"protocol_reachability_compare_{timestamp}.tsv"
...
def main() -> int:
    ...
    if not nodes_path.exists():
        print(f"[protocol-reachability] node file not found: {nodes_path}")
        return 1

    singbox_path = Path(args.singbox)
    if not singbox_path.exists():
        print(f"[protocol-reachability] sing-box not found: {singbox_path}")
        return 1

    urls = [u.strip() for u in args.urls.split(",") if u.strip()]
    if not urls:
        print("[protocol-reachability] no URLs configured")
        return 2
    ...
    for node in nodes:
        ...
        for target in build_targets(node, ip):
            print(f"[protocol-reachability] {safe_label(node)} {target.protocol} ...")
            rows.append(run_reachability_for_target(singbox_path, node, ip, target, urls, args.timeout))

    if not rows:
        print("[protocol-reachability] no benchmarkable protocol records found")
        return 3

    outputs = write_outputs(rows, Path(args.output_dir))
    print(f"[protocol-reachability] JSON: {outputs['json']}")
    print(f"[protocol-reachability] TSV : {outputs['tsv']}")
    return 0
