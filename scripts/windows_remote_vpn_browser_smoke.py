...
def run_local(args, check=True, capture_output=False, env=None):
...
class RemoteWindows:
    ...
    def proxy_fetch(self, url):
        ...
        script = textwrap.dedent(
            f"""
            try {{
              $curl = & curl.exe -L -s -o NUL -w "%{{http_code}} %{{url_effective}}" -x "http://127.0.0.1:20122" "{escaped}"
              $parts = $curl -split ' ', 2
              [pscustomobject]@{{
                success = $true
    ...
    def restore(self, original_user_path, original_proxy):
        smb_cmd = [
            "smbclient",
            f"//{self.host}/C$",
            "-U",
        ...
        proxy_server = str(original_proxy.get("ProxyServer", "")).replace("'", "''")
        script = textwrap.dedent(
            f"""
            Copy-Item 'C:\\Temp\\pd-restore-user.yaml' $env:LOCALAPPDATA\\PrivateDeploy\\data\\user.yaml -Force
            Get-Process PrivateDeploy,sing-box,chrome,iexplore -ErrorAction SilentlyContinue | Stop-Process -Force -ErrorAction SilentlyContinue
            Set-ItemProperty 'HKCU:\\Software\\Microsoft\\Windows\\CurrentVersion\\Internet Settings' ProxyEnable {int(original_proxy.get("ProxyEnable", 0))}
            Set-ItemProperty 'HKCU:\\Software\\Microsoft\\Windows\\CurrentVersion\\Internet Settings' ProxyServer '{proxy_server}'
            $u = Get-Content (Join-Path $env:LOCALAPPDATA 'PrivateDeploy\\data\\user.yaml') -Raw
...
def screenshot(env, window_id, path):
    run_local(["import", "-window", window_id, str(path)], env=env)


def click(env, window_id, x, y):
    run_local(["xdotool", "mousemove", "--window", window_id, str(x), str(y), "click", "1"], env=env)


def key(env, window_id, sequence):
    run_local(["xdotool", "key", "--window", window_id, sequence], check=False, env=env)
...
def global_key(env, sequence):
    run_local(["xdotool", "key", sequence], check=False, env=env)
...
def main():
    ...
    try:
        ...
        original_proxy = prepared["proxy"]
        write_json(output_dir / "original_proxy.json", original_proxy)
        ...
        reachable_failures = sum(1 for item in site_results if not item["proxyFetch"].get("reachable"))
        ...
        summary = {
            "host": args.host,
            "proxy": connected_state["proxy"],
            "listener20122": connected_state["listener20122"],
            ...
            "sites": site_results,
            "overall": "PASS" if app_connected_fails == 0 and reachable_failures == 0 else "PASS_WITH_NOTE",
        }
        write_json(output_dir / "summary.json", summary)
    finally:
        restore_payload = None
        try:
            if original_user_path.exists() and original_proxy:
                restore_payload = remote.restore(str(original_user_path), original_proxy)
        finally:
