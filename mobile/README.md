...
- 没有 `RepairCloudInstance` 桌面 RPC 对应物：iOS 端只能走删除+重建（`cloud_provider.dart.repairInstance` 内部就是 `_createInstance(redeployLabel)`），无法像桌面 SSH provider 那样原地修复同一台机器
- VPN 系统状态 stream 在没有 `VPNCore.framework` 时仅返回静态的 unsupported 事件，不会主动推送状态变化

> 账号状态探测（DO locked / Vultr 防火墙 50/50）已通过 `CloudApiClient.getAccountStatus` 在 iOS+Android 全平台覆盖（纯 Dart 实现）。
