...
class CdnEndpoint {
  ...
  /// 魔戒-class airports pin their entry to a fast, unblocked relay IP. The
  ...
  /// poisoned/blocked. Empty/null = unchanged behaviour (no pinned outbound).
  ...
  /// `*.workers.dev` fallback. On CN cellular both `*.workers.dev` and
  /// the custom domain can be poisoned by different operators in
...
Map<String, String> _appendInstanceOutbounds(
  ...
  if (cdnHost != null &&
    ...
    void addCdnOutbound(String host, String tagSuffix, {String? dialIp}) {
      ...
      // we sidestep CF GeoDNS's throttled CN edge.
      ...
      outbounds.add({
        ...
        'tls': {
          ...
          // → cellular SYN-blocks bare VPS → user reports "still can't
...
String? buildCloudNodeConfig(
  ...
  // active node's IP is dropped by the carrier (e.g. China Mobile silently
  ...
  // (notably China Mobile cellular) every bare-VPS-IP probe burns ~8 s
