...
class CdnProvider with ChangeNotifier {
  ...
  /// SYN-block; mirrors the same checks deployWorkerForNode runs
  ...
  Future<bool> verifyAndPersist(String token) async {
    ...
    try {
      ...
      // motivation as CloudProvider.setApiKey: on CN cellular where this
  ...
  Future<bool> deployWorkerForNode({
    ...
    /// from a cellular SYN-block ('auto'). Persisted on the resulting
    ...
    try {
      ...
      // phone can't reach the DNS-poisoned *.workers.dev host.)
      ...
      if (_customDomain != null && customHost == null && _lastError == null) {
        ...
        // gets DNS-poisoned, the node has ZERO working CDN entry points —
  ...
  Future<void> _probeCustomHostReadiness(String nodeId, String host,
    ...
    for (final d in delays) {
      ...
      // *.workers.dev GET can't (that host is DNS-poisoned on these nets).
  ...
  /// works even where *.workers.dev is DNS-poisoned) and returns the HTTP
  ...
  /// CF/Google follow as fallbacks for non-CN networks where AliDNS is
  /// the slow path.
...
class CdnDeployment {
  ...
  /// SYN-block recovery. Null on records that predate this field so the
