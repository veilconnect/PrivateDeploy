...
void main() {
  ...
  group('Upstream-degraded watchdog', () {
    ...
    test(
        'cellular SYN-block invokes the auto-CDN-deploy handler exactly '
      ...
      // First SYN-block broadcast: handler must fire.
      ...
      expect(invocations, ['Cloud: blocked-node'],
          reason: 'auto-CDN handler must fire on SYN-block');
      ...
      expect(invocations, ['Cloud: blocked-node'],
          reason:
              'second SYN-block on same profile must not re-invoke the handler');
    });

    test(
        'cellular SYN-block error sets needsCdnGuidance until a healthy '
        'connect (or explicit dismiss) clears it', () async {
      expect(vpnProvider.needsCdnGuidance, false);

      // Native side reports the carrier-SYN-block flavor of start failure.
      vpnProvider.debugApplyNativeStatus(VpnNativeStatus(
        running: false,
        status: 'error',
        message: VpnProvider.cellularCarrierSynBlockMessage,
      ...
      expect(vpnProvider.needsCdnGuidance, true,
          reason: 'banner must rise on carrier-SYN-block error broadcast');
    ...
    test('user-initiated restart() resets the watchdog budget', () async {
      ...
      TestDefaultBinaryMessengerBinding.instance.defaultBinaryMessenger
        ...
        switch (call.method) {
          ...
          case 'getEgressIp':
            return {'ip': '198.51.100.14', 'source': 'android_native'};
          case 'isRunning':
            return true;
          default:
            return null;
