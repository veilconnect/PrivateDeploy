...
### 新增
- **可持久恢复的云操作**：创建云主机改用加密、幂等的操作日志；Vultr 和
  DigitalOcean 部署被中断后可继续对账或修复，不再静默丢失节点记录。
- **托管节点修复链路**：补齐 provider 元数据、防火墙所有权、SSH 恢复材料和
  部分完成部署的重启恢复与修复流程；Vultr 和 DigitalOcean 存量主机可原机修复，
  不再通过创建替换主机来“修复”。
- **异步云 API 操作**：独立 REST API 新增云创建操作状态查询和
  `Idempotency-Key` 去重。

### 修复
- **节点长期卡在部署中或显示旧遗留告警**：统一桌面端、API 与移动端的实例状态、
  部署、删除和记录同步。防火墙创建/挂载及服务就绪的瞬时失败会重试；已有托管
  防火墙组会复用；配额错误会立即终止，不再一直停在中间进度。
- **Linux 白屏和启动不稳定**：在 WebKitGTK 构造阶段落实渲染策略，增加软件渲染
  回退，将 GUI 就绪状态与实际启动进程关联，并把本地安装改为事务式更新。
  Ubuntu/Jammy AppImage 已在真实桌面完成非白屏验证。
- **重启后静默退回 Direct**：只要存在可用云节点，就重新生成 sing-box 运行配置，
  不再错误降级成仅 Direct 配置。
- **移动端 VPN 重连循环和蜂窝 DNS 失效**：瞬时探测失败、单一 CDN 被阻断、HTTP
  403 或国内直连探测假阴不再拆除正常隧道；同时加固 DNS、CDN 优先探测、失效
  节点切换、VLESS SNI 自愈及蜂窝 CDN 行为。
- **CDN relay 可靠性**：边缘发现不再依赖可能被污染的自定义域名 DNS，并取消会让
  Cloudflare relay 出现 522 的 UFW 端口限速。

### 安全
- 独立 API 默认使用持久随机 token；远程监听必须显式配置 token；无认证模式仅限
  回环地址，并收紧 CORS/WebSocket Origin 校验。
- 远程插件要求明确确认“完全信任”，只允许 HTTPS 白名单来源，逐跳复核重定向并用
  SHA-256 固定代码；离线缓存和自动备份会移除凭据及分享材料，读取旧数据时也会清理。
- 加固 provider 记录加密持久化、安全存储及路径/符号链接边界、云操作并发锁、
  渲染器网络请求的 SSRF/重绑定防护和错误脱敏。

### 变更
- Jammy 发布构建改为非交互和冻结前端依赖；Wails 补丁不再污染工作区；容器构建
  后会恢复产物所有权，同时保留原始构建退出码。
- 扩充签名发布、来源证明、校验和、SBOM、许可证审计及只读云端 gate 自动化。

### 测试
- 新增云操作恢复、provider 并发、防火墙生命周期、启动就绪、Linux 渲染策略、
  事务式安装和 AppImage GUI 进程关联的回归覆盖。
